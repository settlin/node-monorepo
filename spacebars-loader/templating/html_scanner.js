  ...
  _handleTag: function (results, tag, attribs, contents, throwParseError,
    ...
    try {
      if (tag === "template") {
        ...
        results.js +=
          '\nmodule.exports.template = new Blaze.Template(' +
          (options.templateName || templateDotNameLiteral) + ", " +
          renderFuncCode + ");\n";

        if (options.attachGlobal) {
          results.js +=
            "Blaze.Template.__checkName(" + nameLiteral + ");\n" +
            'Blaze.Template[' + nameLiteral + '] = module.exports.template;\n';
        }
        // }}}
      } else {
        // <body>
        ...
        results.js += "\nBlaze.Template.body.addContent(" + renderFuncCode + ");\nMeteor.startup(Blaze.Template.body.renderToDocument);\n";
      }
    } catch (e) {
      if (e.scanner) {
