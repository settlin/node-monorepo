...
const Input = (props) => {
  const extraProps = () => {
    let { type: typeOrig, label = '', required, validate: validateOrig, compact = true, formik = true, indian } = props;

    if (required && compact && label) label = label.replace(/\*$/, '').trim() + ' *';
    if (!formik) return { label };

    let validateFunc = () => { }, validateReq = () => { };
    if (typeof validateOrig === 'function') validateFunc = validateOrig;
  ...
  const module = () => {
    let { base, type, formik = true, options, components: { input } = {} } = props;
  ...
  const {
    ...
    formik = true,
    base,
    components: { input, Field = module(), Loader = LinearProgress, ...components } = {},
    fast = true,
    compact = true,
    ...rest
  } = props;
  ...
  const extraPropsData = { ...(formik ? { fast, ...(input ? { component: input } : {}) } : {}), compact, components, ...extraProps() };

  return (
    <ErrorBoundary>
      <Container {...containerProps}>
        {Field
          ? <Field {...rest} {...(inputType ? { type: inputType } : {})} {...extraPropsData} />
          : <Loader />
        }
      </Container>
    </ErrorBoundary>
