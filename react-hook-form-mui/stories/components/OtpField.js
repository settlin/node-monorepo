...
import TextField from '../../src/react-hook-form/TextField';
...
class ResendButton extends React.PureComponent {
	...
	render() {
		...
		return (
<FButton size='small' variant='text' disabled={Boolean(remainingSecs)} onClick={onClick} processing={processing}>
...
InputProps={{
									endAdornment: <InputAdornment><IconButton aria-label='Cancel Otp' onClick={hCancelOtp} size="large"><Clear /></IconButton></InputAdornment>,
								}}/>
							)
							: phones.length
								? <Select {...props} defaultValue={phones[0].value} options={phones}/>
								: <TextField {...props} onKeyDown={hEnterOnMobile} type='number'/>
					}
				</Grid>
	{!verified && (
<Grid container item xs={12} justifyContent={sent ? 'space-between' : 'flex-end'}>
					{sent
						? (
							<React.Fragment>
